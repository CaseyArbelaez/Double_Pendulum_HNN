...
from src.models import HamiltonianNN
from src.training import hnn_vector_field_loss
from src.utils import load_dataset_npz, plot_training_curves, save_dataset_npz, set_seed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train HNN on double pendulum data.")
    ...
    parser.add_argument("--outdir", type=str, default=str(ROOT / "artifacts" / "hnn"))
...
def evaluate(model: torch.nn.Module, loader: DataLoader, device: torch.device) -> float:
    ...
    for xb, yb in loader:
        ...
        yb = yb.to(device)
        loss = hnn_vector_field_loss(model, xb, yb)
...
def main() -> None:
    ...
    model = HamiltonianNN(hidden_dim=args.hidden_dim, depth=args.depth).to(device)
    ...
    plot_training_curves(history, outdir / "loss_curve.png", "Hamiltonian NN training")
